...
from apm import config, plugins
from apm import loggings
from apm import profile
from apm.agent.protocol import Protocol
from apm.command import command_service
from apm.loggings import logger
from apm.profile.profile_task import ProfileTask
from apm.profile.snapshot import TracingThreadSnapshot

if TYPE_CHECKING:
    from apm.trace.context import Segment
...
def __init():
    ...
    if config.protocol == 'grpc':
        from apm.agent.protocol.grpc import GrpcProtocol
        __protocol = GrpcProtocol()
    elif config.protocol == 'http':
        from apm.agent.protocol.http import HttpProtocol
        __protocol = HttpProtocol()
    elif config.protocol == 'kafka':
        from apm.agent.protocol.kafka import KafkaProtocol
