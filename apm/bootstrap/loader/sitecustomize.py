...
def _get_sw_loader_logger():
    """ Setup a new logger with passed apm CLI env vars,
    don't import from apm, it may not be on sys.path
    if user misuses the CLI to run programs out of scope
    """
    from logging import getLogger
    logger = getLogger('apm-loader')
...
# Even if `import apm` was successful, doesn't mean its the same one where sw-python CLI was run.
...
if not (version_match and prefix_match):
    ...
    _sw_loader_logger.error('The sw-python CLI was instructed to run a program '
                            ...
                            'Please make sure that sw-python cli, apm agent and your '
    ...
    from apm import agent, config

    # also override debug for apm agent itself
    if os.environ.get('SW_PYTHON_CLI_DEBUG_ENABLED') == 'True':  # set from the original CLI runner
