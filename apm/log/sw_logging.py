...
from apm.protocol.logging.Logging_pb2 import LogData, LogDataBody, TraceContext, LogTags, TextLog

from apm import config, agent
from apm.trace.context import get_context
from apm.utils.filter import sw_traceback, sw_filter
...
def install():
    ...
    if layout:
        from apm.log.formatter import SWFormatter
    ...
    def _sw_handle(self, record):
        if record.name in ['apm', 'apm-cli', 'apm-loader']:  # Ignore SkyWalking internal loggers
